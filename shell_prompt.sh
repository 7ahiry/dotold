...
function __promptline_ps1 {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "y" slices
  __promptline_wrapper "$(__my_promptline_vcs_branch)$(git_super_status)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __my_promptline_vcs_branch {
  local branch
  local branch_symbol=" "
  ...
  if hash git 2>/dev/null; then
    if branch=$( { git symbolic-ref --quiet HEAD || git rev-parse --short HEAD; } 2>/dev/null ); then
      branch=${branch##*/}
      #printf "%s %s" "${branch_symbol}${branch:-unknown} ${git_super_status}"
...
function __promptline_right_prompt {
  ...
  # section "y" slices
  __promptline_wrapper "$(__my_promptline_vcs_branch)$(git_super_status)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
